...
export const Hero = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', calculateWaveHeights);
    return () => window.removeEventListener('resize', calculateWaveHeights);
  }, []);

  const calculateWaveHeights = () => {
    ...
    if (width < 640) { // Mobile
      ...
      amplitude = 50;
      baseHeight = 30;
    } // Desktop keeps default values
    
    const heights = [...Array(70)].map((_, i) => 
      Math.sin(i * 0.3) * amplitude + baseHeight
    );
    setWaveHeights(heights);
  };

  // Initial static heights for SSR (mobile-first)
  const getStaticHeight = (i) => {
    return Math.sin(i * 0.3) * 20 + 15; // Default mobile values
  };
    ...
    <div className="py-4 sm:py-5 md:py-6 lg:py-8">
      <div className="max-w-4xl mx-auto h-auto min-h-[70vh] sm:min-h-[75vh] md:min-h-[80vh] lg:h-[90vh] px-3 sm:px-4 md:px-6">
        <div className="mt-8 sm:mt-12 md:mt-16 lg:mt-20">
          ...
          {/* Wave visualization - Responsive */}
          <div className="h-12 sm:h-14 md:h-16 lg:h-18 flex items-end justify-center gap-0.5 sm:gap-1 px-1 sm:px-2 overflow-x-auto md:overflow-visible">
            {[...Array(70)].map((_, i) => (
              <div
                key={i}
                className="w-1 sm:w-1.5 md:w-2 rounded-t-sm sm:rounded-t-md md:rounded-t-lg bg-gradient-to-t from-blue-500/50 to-purple-500/50"
                style={{
          ...
          {/* Description text */}
          <p className="text-center text-pretty text-sm sm:text-base md:text-lg lg:text-xl mb-4 sm:mb-6 mx-auto text-indigo-100 opacity-0 animate-fadeIn animation-delay-700 max-w-xs sm:max-w-sm md:max-w-xl lg:max-w-2xl px-2 sm:px-0">
          ...
          <div className="flex justify-center items-center gap-3 sm:gap-4 mx-auto max-w-fit py-3 sm:py-4 md:py-5 opacity-0 animate-fadeIn animation-delay-900">
            <Link href="/edit-audio" className="w-full sm:w-auto">
              <button className="group relative inline-flex items-center justify-center w-full sm:w-auto px-4 sm:px-5 md:px-6 py-2.5 sm:py-3 md:py-3.5 font-medium text-white rounded-xl sm:rounded-2xl border hover:scale-[1.02] sm:hover:scale-[1.05] active:scale-[0.98] transition-transform duration-200">
                {/* Shiny overlay */}
      ...
      <style jsx>{`
        ...
        /* Responsive font size adjustments for very small screens */
